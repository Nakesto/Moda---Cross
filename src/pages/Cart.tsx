...
const Cart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "cart", userData!.uid), (doc) => {
      setCart(
        Object.entries(doc.data() as any).sort((a: any, b: any) => {
          {
            const nameA = a[1].product.name.toUpperCase(); // ignore upper and lowercase
            const nameB = b[1].product.name.toUpperCase(); // ignore upper and lowercase
            if (nameA < nameB) {
              return -1;
            }
            if (nameA > nameB) {
